...
class LSTM(nn.Module):
    ...
    def forward(self, input, hidden, cell_state):
        ...
        output_gate = torch.sigmoid(self.i2g(combined))
...
def unicodeToAscii(s): return ''.join(
...
for iter in range(1, n_iters + 1):
    ...
    hidden, cell_state = lstm.initHidden()

    lstm.zero_grad()

    for i in range(input.size()[0]):
        output, hidden, cell_state = lstm(input[i], hidden, cell_state)
