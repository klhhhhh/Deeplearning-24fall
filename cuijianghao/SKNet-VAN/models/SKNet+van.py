...
class SKAttentionConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, M=2, r=16, L=32, dropout_rate=0.5):
        super(SKAttentionConv, self).__init__()
        self.SKConv = SKConv(in_channels, out_channels, stride, M, r, L)
        self.AttentionBlock = VisualAttentionBlock(out_channels, out_channels)
...
class SKAttentionNet(nn.Module):
    def __init__(self, num_classes=100, dropout_rate=0.4):
        ...
        self.dropout = nn.Dropout(dropout_rate)
        self.skattconv1 = SKAttentionConv(64, 64)
        self.skattconv2 = SKAttentionConv(64, 128, stride=2)
        self.skattconv3 = SKAttentionConv(128, 256, stride=2)
    ...
    def forward(self, x):
        ...
        out = self.skattconv1(out)
        out = self.dropout(out)
        out = self.skattconv2(out)
        out = self.dropout(out)
        out = self.skattconv3(out)
        out = self.dropout(out)
