...
class AttentionSKNet(nn.Module):
    def __init__(self, num_classes=100, dropout_rate=0.5):
        super(AttentionSKNet, self).__init__()

        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
        self.bn1 = nn.BatchNorm2d(64)
        self.dropout = nn.Dropout(dropout_rate)

        # Add attention block and SKConv block in an alternating pattern
        self.att_block1 = VisualAttentionBlock(64, 128, dropout_rate=dropout_rate)
        self.skconv1 = SKConv(128, 128)

        self.att_block2 = VisualAttentionBlock(128, 256, dropout_rate=dropout_rate)
        self.skconv2 = SKConv(256, 256)

        self.att_block3 = VisualAttentionBlock(256, 512, dropout_rate=dropout_rate)
        self.skconv3 = SKConv(512, 512)
    ...
    def forward(self, x):
        ...
        out = self.att_block1(out)
        out = self.skconv1(out)
        out = self.dropout(out)

        out = self.att_block2(out)
        out = self.skconv2(out)
        out = self.dropout(out)

        out = self.att_block3(out)
