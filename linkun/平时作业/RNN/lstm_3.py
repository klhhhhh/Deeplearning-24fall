...
def train(category_tensor, line_tensor):
    ...
    lstm.zero_grad()
    input = line_tensor.reshape(1, -1, 57)
    output= lstm(input)
...
for iter in range(1, n_iters + 1):
    ...
    if iter % print_every == 0:
        ...
        print('\nValidation set: current loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        avg_loss / print_every, avg_correct, print_every, 100 * avg_correct / print_every))
        avg_loss = 0
        avg_correct = 0
    # Add current loss avg to list of losses
    if iter % plot_every == 0:
        all_losses.append(current_loss / plot_every)
        all_accuracy.append(100 * current_correct / plot_every)
        current_loss = 0
        current_correct = 0
